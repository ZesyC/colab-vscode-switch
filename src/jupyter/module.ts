...
import { QuotaFailoverController } from '../colab/quota-failover';
...
export function createJupyterModule(
  ...
  // Servers are stored per account, and the scope is read on every access so
  // switching the active account immediately swaps in that account's servers.
  const serverStorage = new ServerStorage(
    vs,
    context.secrets,
    () => authProvider.activeAccountEmail,
  );
  const assignmentManager = new AssignmentManager(
    ...
    colabApiClient,
    serverStorage,
    new QuotaFailoverController(vs, authProvider),
  );

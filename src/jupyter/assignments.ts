...
import { AccountFailover } from '../colab/quota-failover';
...
export class AssignmentManager implements Disposable {
   ...
   * @param failover - Switches to another signed-in Google account when the
   * active one runs out of quota. Omit to disable multi-account failover.
   */
  constructor(
    ...
    private readonly failover?: AccountFailover,
  ) {
  ...
  async assignServer(
    ...
    return this.assignServerWithFailover(descriptor, new Set(), signal);
  }

  /**
   * Assigns a server, falling back to another Google account when the active
   * one is out of quota.
   *
   * @param descriptor - The server descriptor used as a template for the server
   * being assigned. It is reused verbatim on the fallback account, so the
   * accelerator, machine shape, runtime version and alias all carry over.
   * @param triedAccounts - Session IDs already known to be out of quota during
   * this assignment, so each account is only attempted once.
   * @param signal - The cancellation signal.
   * @returns The assigned server.
   */
  private async assignServerWithFailover(
    descriptor: ColabServerDescriptor,
    ...
    try {
      ...
      try {
        ...
        if (error instanceof InsufficientQuotaError) {
          const onFallbackAccount = await this.tryFailoverToNextAccount(
            descriptor,
            triedAccounts,
            signal,
          );
          if (onFallbackAccount) {
            return onFallbackAccount;
          }
  ...
  /**
   * Re-assigns the same server on the next signed-in Google account after the
   * active one ran out of quota.
   *
   * The exhausted account's servers are released first (when configured), both
   * to free its resources and to stay under Colab's per-account assignment
   * limit. Doing so destroys whatever is on those servers, which is why it is
   * behind the `colab.multiAccount.releaseServersOnSwitch` setting.
   *
   * @param descriptor - The server template to re-create on the next account.
   * @param triedAccounts - Accounts already known to be out of quota.
   * @param signal - The cancellation signal.
   * @returns The server assigned on the fallback account, or undefined when no
   * failover happened.
   */
  private async tryFailoverToNextAccount(
    descriptor: ColabServerDescriptor,
    triedAccounts: ReadonlySet<string>,
    signal?: AbortSignal,
  ): Promise<ColabAssignedServer | undefined> {
    ...
    const exhausted = new Set(triedAccounts);
    const active = failover.activeAccountId();
    if (active) {
      exhausted.add(active);
    }
    if (!failover.hasAlternatives(exhausted)) {
      return undefined;
    }
    if (failover.shouldReleaseServers()) {
      await this.releaseAllServers(signal);
    }
    const switched = await failover.switchToNext(
      exhausted,
      'The active Colab account is out of quota.',
    );
    if (!switched) {
      return undefined;
    }
    return this.assignServerWithFailover(descriptor, exhausted, signal);
  }

  /**
   * Best-effort deletion of every server assigned to the active account.
   *
   * Failures are logged rather than thrown: this runs while recovering from an
   * assignment failure, and a stuck server must not block the failover.
   *
   * @param signal - The cancellation signal.
   */
  private async releaseAllServers(signal?: AbortSignal): Promise<void> {
    let servers: readonly ColabAssignedServer[];
    try {
      // Read storage directly rather than going through getServers(), which
      // reconciles against the backend. Reconciliation issues requests as the
      // account that just failed, and we want to delete everything we recorded
      // regardless of what that listing would say.
      servers = await this.storage.list();
    } catch (err: unknown) {
      log.warn(
        'Unable to list servers to release before switching account',
        err,
      );
      return;
    }
    for (const server of servers) {
      try {
        await this.unassignServer(server, signal);
      } catch (err: unknown) {
        log.warn(
          `Failed to release server ${server.id} before switching account`,
          err,
        );
      }
    }

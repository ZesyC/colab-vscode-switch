  ...
  describe('assignServer', () => {
    describe('with Public API disabled', () => {
      ...
      describe('with insufficient quota', () => {
        ...
        describe('with another account available', () => {
          let failover: FakeAccountFailover;

          /**
           * Wires a Jupyter client stub for a server so that unassigning it
           * can list (and find no) sessions to clean up.
           *
           * @param server - The server whose connection should be stubbed.
           */
          function stubJupyterConnectionFor(server: ColabAssignedServer) {
            const jupyterStub = createJupyterClientStub();
            (jupyterStub.sessions.list as sinon.SinonStub).resolves([]);
            jupyterStaticConnectionStub
              .withArgs(
          ...
          beforeEach(() => {
            failover = new FakeAccountFailover(['acct-1', 'acct-2']);
            assignmentManager = new AssignmentManager(
              vsCodeStub.asVsCode(),
              colabClientStub,
              colabApiClientStub,
          ...
          it('switches account and re-assigns with the same configuration', async () => {
            // Only the second account has quota.
            colabClientStub.assign
              .onSecondCall()
              .resolves({ assignment: defaultAssignment, isNew: true });

            const server = await assignmentManager.assignServer(
              defaultAssignmentDescriptor,
            );

            expect(failover.activeAccountId()).to.equal('acct-2');
            expect(server.variant).to.equal(
              defaultAssignmentDescriptor.variant,
            );
            expect(server.accelerator).to.equal(
              defaultAssignmentDescriptor.accelerator,
            );
            expect(server.label).to.equal(defaultAssignmentDescriptor.label);
          });

          it('gives up once every account is out of quota', async () => {
            await expect(
              assignmentManager.assignServer(defaultAssignmentDescriptor),
            ).to.eventually.be.rejectedWith(InsufficientQuotaError);

            // Both accounts were attempted exactly once.
            expect(failover.switchCount).to.equal(1);
            sinon.assert.calledTwice(colabClientStub.assign);
          });
          ...
          it('does not switch when auto-switch is disabled', async () => {
            ...
            expect(failover.switchCount).to.equal(0);
            expect(failover.activeAccountId()).to.equal('acct-1');
          });

          it('releases the servers of the exhausted account before switching', async () => {
            await serverStorage.store([defaultServer]);
            stubJupyterConnectionFor(defaultServer);
            colabClientStub.assign
              .onSecondCall()
              .resolves({ assignment: defaultAssignment, isNew: true });
          ...
          it('keeps the servers when releasing is disabled', async () => {
            failover.releaseServers = false;
            await serverStorage.store([defaultServer]);
            stubJupyterConnectionFor(defaultServer);
            colabClientStub.assign
              .onSecondCall()
...
/**
 * An in-memory {@link AccountFailover} that cycles through a fixed list of
 * account IDs.
 */
class FakeAccountFailover implements AccountFailover {
  enabled = true;
  releaseServers = true;
  switchCount = 0;
  private index = 0;
  ...
  hasAlternatives(exclude: ReadonlySet<string>): boolean {
    return this.ids.some(
      (id) => id !== this.activeAccountId() && !exclude.has(id),
    );
  }

  switchToNext(exclude: ReadonlySet<string>): Promise<string | undefined> {
    const next = this.ids.findIndex(
      (id) => id !== this.activeAccountId() && !exclude.has(id),
    );
    if (next < 0) {
    ...
    this.index = next;
    this.switchCount++;
    return Promise.resolve(this.ids[next]);
  }
}

...
import { addAccount, manageAccounts, switchAccount } from './accounts';
import {
  ADD_ACCOUNT,
  COLAB_TOOLBAR,
  MANAGE_ACCOUNTS,
...
export function registerColabCommands(
  ...
  return [
    ...
    registerCommand(vs, SIGN_OUT_ALL.id, async () => {
      await authProvider.signOutAll();
    }),
    registerCommand(vs, ADD_ACCOUNT.id, async () => {
      await addAccount(authProvider);
    }),
    registerCommand(vs, SWITCH_ACCOUNT.id, async () => {
      await switchAccount(vs, authProvider);
    }),
    registerCommand(vs, MANAGE_ACCOUNTS.id, async () => {
      await manageAccounts(vs, authProvider);
    }),

...
describe('GoogleAuthProvider', () => {
  ...
  /**
   * Builds a provider over the shared stubs. The single stateful OAuth2 client
   * is reused for every account, which is what the stubs of its methods rely
   * on.
   *
   * @returns A fresh, uninitialized provider.
   */
  function newAuthProvider(): GoogleAuthProvider {
  ...
  /**
   * Replaces the provider with one that has already signed the given accounts
   * in, so tests can exercise behavior that depends on an active account.
   *
   * @param sessions - The sessions to restore from storage.
   */
  async function reinitializeWith(
    sessions: RefreshableAuthenticationSession[],
  ): Promise<void> {
    storageStub.getSessions.resolves(sessions);
    if (!('restore' in oauth2Client.refreshAccessToken)) {
  ...
  describe('lifecycle', () => {
    ...
    describe('initialize', () => {
      ...
      it('activates one of several stored accounts', async () => {
        const second: RefreshableAuthenticationSession = {
          ...DEFAULT_REFRESH_SESSION,
          id: '9',
          account: { id: 'bar@example.com', label: 'Bar' },
        };
        storageStub.getSessions.resolves([DEFAULT_REFRESH_SESSION, second]);
        sinon.stub(oauth2Client, 'refreshAccessToken').callsFake(() => {
          oauth2Client.credentials.access_token = DEFAULT_ACCESS_TOKEN;
          return undefined as never;
        });
        ...
        expect(authProvider.accountCount).to.equal(2);
        expect(authProvider.activeAccountEmail).to.equal(
          DEFAULT_REFRESH_SESSION.account.id,
        );
      });
    });
  });
  ...
  describe('createSession', () => {
    ...
    describe('with a successful login', () => {
      ...
      it('replaces an existing session', async () => {
        await reinitializeWith([DEFAULT_REFRESH_SESSION]);
      ...
      it('adds an account with the account chooser forced', async () => {
        loginStub
          .withArgs(SCOPES, {
            includeGrantedScopes: false,
            loginHint: undefined,
          ...
          .resolves(DEFAULT_CREDENTIALS);

        const session = await authProvider.addAccount(SCOPES);

        expect(session.account.id).to.equal(DEFAULT_REFRESH_SESSION.account.id);
        sinon.assert.calledOnceWithExactly(loginStub, SCOPES, {
          includeGrantedScopes: false,
          loginHint: undefined,
          forceAccountChooser: true,
...
/**
 * Builds an in-memory {@link vscode.Memento} for the active-account pointer.
 *
 * @returns A memento backed by a plain map.
 */
function newMementoStub(): vscode.Memento {

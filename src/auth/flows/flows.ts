...
export interface OAuth2TriggerOptions {
   ...
   *
   * If the value is "select_account consent" - the account chooser is shown
   * first, allowing the user to authorize a *different* Google account than the
   * one already signed in. Used when adding an additional account.
   *
   * This should always be "consent" unless `includeGrantedScopes` is `true`
   * to allow for incremental auth.
   */
  readonly prompt?: 'consent' | 'select_account consent';
}
